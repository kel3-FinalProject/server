...
class Controller {
  ...
  static async createReservasi(req, res) {
    ...
    try {
      ...
      if (!kamar) {
        await t.rollback();
        return res.status(404).json({ error: 'Kamar tidak ditemukan. Silahkan pilih kamar lain.' });
      }
      ...
      if (existingReservasi) {
        await t.rollback();
        return res.status(400).json({ error: 'Kamar sudah direservasi. Silahkan pilih kamar lain.' });
      }
      ...
      const selisihHari = Math.ceil((new Date(tanggal_checkout) - new Date(tanggal_checkin)) / (1000 * 60 * 60 * 24));
  ...
  static async bayarReservasi(req, res) {
    try {
      ...
      if (reservasi.status !== 'menunggu pembayaran') {
        return res.status(400).json({ message: 'Reservasi tidak dapat dibayar. Status sudah berubah.' });
      }
      ...
      res.status(200).json({ message: 'Pembayaran berhasil. Reservasi telah diproses.' });
    } catch (error) {
  ...
  static async updateReservasiStatus(req, res) {
    ...
    try {
      ...
      res.status(200).json({ message: `Status reservasi berhasil diubah menjadi ${status}` });
    } catch (error) {
